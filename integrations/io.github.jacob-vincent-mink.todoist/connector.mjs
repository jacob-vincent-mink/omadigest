...
export async function handle(request, fetchImpl = fetch) {
  ...
  const range = completionRange(request.since, request.until);
  const [due, assigned, completed] = await Promise.all([
    todoistJson("/api/v1/tasks/filter", { query: "overdue | today | next 7 days", limit: String(Math.min(200, limit * 2)) }, token, fetchImpl),
    todoistJson("/api/v1/tasks/filter", { query: "assigned to: me", limit: String(Math.min(200, limit * 2)) }, token, fetchImpl),
    todoistJson("/api/v1/tasks/completed/by_completion_date", { since: range.since, until: range.until, limit: String(limit) }, token, fetchImpl)
  ]);
  const items = parseTodoistData({ due, assigned, completed }, request.since, request.until, limit);
  emit({ version: 1, type: "items", id: request.id, items, nextCursor: null });
  return true;
}

export function parseTodoistData({ due, assigned, completed }, sinceValue, untilValue, limit = MAX_ITEMS, nowValue = new Date()) {
  ...
  for (const task of pageItems(due, "results").slice(0, 100)) {
    ...
    add(task, label, task?.updated_at || `${date}T00:00:00Z`, label === "overdue" ? `Overdue since ${date}` : date === today ? "Due today" : `Due ${date}`);
  }
  for (const task of pageItems(assigned, "results").slice(0, 100)) add(task, "assigned", task?.updated_at || task?.added_at, task?.due?.date ? `Assigned to you · due ${bounded(task.due.date, 40)}` : "Assigned to you");
  for (const task of pageItems(completed, "items").slice(0, 50)) {
