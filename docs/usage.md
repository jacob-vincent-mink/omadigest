...
Open **Recurring research** at the top of Sources to schedule a recurring public-web question. Give it a name, the question to keep current, an hourly-or-slower cadence, a research depth, a change window, and up to eight preferred HTTPS sources. Focused research may use 4 searches/12 pages, broad 10/30, and deep 20/60; corpus limits still prevent a collection from overrunning the model context. New watches default to broad research over the last 30 days. The first run uses up to 30 days of current evidence to establish a useful cited baseline even when the selected change window is shorter. Later runs use the selected window, narrowed to the last successful snapshot when more recent. Searches must remain on-topic and are bounded both before and after the effective as-of date. Existing claims stay in the current picture until current evidence explicitly replaces or retires them; failure to rediscover a claim is not a change. A partial crawl preserves the last good picture, creates no digest, records why validation stopped, and retries within an hour. Existing cards expose depth, change window, and a two-step **Rebuild baseline** action behind their settings button. The question, date-aware search terms, and fetched public pages leave the computer; pages are treated as untrusted evidence rather than instructions.

Digest titles are capped at 80 characters, headlines at 120, and explanations at 220. Entries lead with the outcome, keep one idea per headline, and avoid repeating the headline in the explanation. Research briefs keep atomic claims in the ledger but group current-picture claims that share the same cited source set.
...
Open **Settings → Behavior** to manage temporary next reviews, create persistent attention rules, and search retained memory. A next review is a bounded follow-up OmaDigest schedules while waiting for related evidence or a deadline. An attention rule is a user-authored instruction that stays enabled for recurring matching situations and deterministically chooses ignore, hold, digest, or notify. A new rule is shown as a compact preview of its current matches and priority overlaps before **Add rule** persists it. The calibration card summarizes only explicit reads, handoffs, and useful/not-useful feedback, including the bounded direction OmaDigest may use as a soft timing hint.
...
Open **Settings → Data** to delete digest history, OmaDigest's retained notification evidence, recurring research and claim history, custom integrations, or templates, or to delete all stored OmaDigest data. Every action requires confirmation. Template deletion here also restores packaged defaults hidden inline.

Deleting notification history affects only OmaDigest state. It records an OmaDigest-owned cutoff so older notifications still present in Omarchy are not imported again; Omarchy's own notification history is never changed. Deleting integrations removes custom packages, setup, enablement, and known integration secrets while leaving bundled packages available. **Delete all** also removes attention rules, but does not remove model authentication or privacy rules.
