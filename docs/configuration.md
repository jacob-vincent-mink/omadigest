...
Bounded runtime state lives separately under `${XDG_STATE_HOME:-$HOME/.local/state}/omadigest/`. That directory includes attention segments, seen IDs, digest history, research claim history, the bounded attention-loop watch/decision ledger, a 512-KiB/512-episode/90-day progressive attention memory, dismissed template suggestions, and one release-update cache record. It is product state, not an agent-editable control plane.
...
├── research-watches.json           # up to 16 hourly-or-slower public research schedules
├── templates/<id>/
