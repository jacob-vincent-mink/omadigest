...
Attention items carry stable ID, source, app, bounded title/body, content-availability state, urgency, timestamp, and at most four credential-free HTTPS locators. QML submits only the bounded live popup snapshot; the broker independently reads at most 50 regular non-symlink Omarchy history files under 64-KiB/file and 512-KiB total limits. Both paths canonicalize a native notification to the same timestamp/original-ID key before the broker rewrites duplicate legacy records. The broker keeps at most 500 items in memory and seven daily mode-`0600` JSONL segments, deduplicates identical snapshots before append, and enforces 2-MiB segment and 8-MiB total budgets. Count-only records never reach either model. The broker deterministically extracts stable entities, recognizes bounded chat application/title threads, matches validated standing policies, and detects bounded future meeting/deadline times before deliberation. A separate mode-`0600` episodic memory retains at most 512 provenance-preserving evidence, decision, digest, and outcome records for 90 days under 512 KiB; its time-decayed summary tree is derived and rebuildable. The attention agent receives only grouped actionable evidence, a bounded temporal cover, soft outcome-derived preferences, public template summaries, and the results of up to four broker-owned recall calls. A digest selection is expanded to complete correlated groups before the digest agent sees it. The digest agent compiles those groups into the minimum useful outcome-level entries; deterministic validation prevents group splitting, partial group citations, and source reuse. When a digest is accepted, the broker snapshots only its cited source destinations into the bounded digest record, so web jump-backs survive attention retention while notification app focus remains explicitly best-effort. A recent single-thread conversation digest may be atomically replaced with a newly collated version; mixed or feedback-bearing digests are excluded.
...
User privacy rules, standing attention policies, templates, integrations, declared permissions, enablement, category overrides, and non-secret setup live under `${XDG_CONFIG_HOME:-~/.config}/omadigest`. The bounded attention ledger also owns each review notice's hidden state, keeping presentation dismissal separate from cancellation and model-visible outcomes. Standing policies are schema-validated, capped at 32 records and 128 KiB, and matched deterministically. Source enablement and category overrides use bounded version-2 state; version-1 integration enablement is migrated on read and rewritten on the next state change. The broker fingerprints this bounded tree every two seconds. Valid edits made by the default Omarchy agent or another editor are reloaded and published to QML without restarting the shell. Secrets remain outside this control plane in Secret Service, and provider account changes remain behind typed authentication.

Research watch definitions also live in that bounded control plane. The broker owns cadence, one-at-a-time execution, depth and change-window profiles, a weighted daily automatic-work budget, public-HTTPS/DNS validation, response and corpus limits, and a 90-day claim ledger capped at 12 runs per watch. Search queries must retain specific watch terms and include lower and upper date boundaries; a first baseline gets a 30-day evidence floor. Off-topic discovery results never become readable candidates. Fetched pages expose bounded publication metadata, falling back to a news-feed publication date and then HTTP `Last-Modified` when the page omits one; ordinary web-search crawl timestamps are not treated as publication dates. The model emits stable cited claims and evidence-backed explicit retirements; TypeScript rejects future-dated or off-topic evidence, carries omitted claims forward, computes validated deltas, and turns only a baseline or meaningful delta into a normal unread digest. Partial crawls preserve the latest complete ledger, emit no digest, record a bounded diagnostic, and retry in one hour.
...
- bounded digest-source lookup and broker-owned web/application launch by digest, entry, source, and target ID;
