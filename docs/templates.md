...
`template.compiled.json` is not intended for routine hand editing. It contains bounded fields validated by `runtime/src/template-schema.ts`:
...
The drafting session can call only `emit_template_draft` or `out_of_scope`. The host validates the proposal before it reaches the review surface. It cannot enable integrations, access notification content, change model credentials, execute commands, or install itself.
