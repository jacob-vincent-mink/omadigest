...
- **Timely event prep** — approaching meetings and deadlines can schedule a bounded pre-event checkback, recall related history, and produce a focused briefing.
- **Attention rules** — describe what should interrupt, wait, become a digest, or stay quiet; a scoped compiler previews current matches and priority conflicts before the deterministic rule is saved.
